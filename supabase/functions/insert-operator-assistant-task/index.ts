...
import { insertTaskForOperatorAssistant } from "../_shared/operatorStudioAssistant/insertOperatorAssistantTaskCore.ts";
import { validateOperatorAssistantTaskPayload } from "../_shared/operatorStudioAssistant/validateOperatorAssistantTaskPayload.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const validated = validateOperatorAssistantTaskPayload(body);
    if (!validated.ok) {
    ...
    console.error(JSON.stringify({ type: "insert_operator_assistant_task_failed", message: msg }));
